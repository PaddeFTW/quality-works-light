...
export function ManualWorkspace() {
  ...
  const [settings, setSettings] = useState({ title: "Kvalitetsmanual", owner: "Kvalitetsansvarig" });
      ...
      <div className="grid min-h-0 flex-1 gap-6 xl:grid-cols-[260px_minmax(0,1fr)]">
        ...
        <main className="min-w-0 rounded-2xl border bg-card shadow-token-sm">
          ...
          <Tabs className="p-5 sm:p-6" onValueChange={setActiveTab} value={activeTab}>
            ...
            <TabsContent className="mt-6" value="work"><Card><CardHeader><CardTitle>Vår version</CardTitle><CardDescription>Anpassa innehållet så att det passar vår verksamhet.</CardDescription></CardHeader><CardContent className="flex flex-col gap-4"><Textarea className="min-h-72 leading-7" onChange={(event) => setDraft(event.target.value)} value={draft} /><div className="flex items-center justify-between"><p className="text-xs text-muted-foreground">Ändringar sparas automatiskt i den här vyn.</p><Button onClick={() => setAcknowledged(true)}>{acknowledged ? <Check data-icon="inline-start" /> : null}{acknowledged ? "Sparad" : "Spara ändringar"}</Button></div></CardContent></Card></TabsContent>
            <TabsContent className="mt-6" value="settings"><Card><CardHeader><CardTitle>Inställningar</CardTitle><CardDescription>Hantera manualens grundläggande information.</CardDescription></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2"><label className="flex flex-col gap-2 text-sm font-medium">Titel<Input onChange={(event) => setSettings({ ...settings, title: event.target.value })} value={settings.title} /></label><label className="flex flex-col gap-2 text-sm font-medium">Ansvarig<Input onChange={(event) => setSettings({ ...settings, owner: event.target.value })} value={settings.owner} /></label><div className="sm:col-span-2 flex items-center gap-2 rounded-lg bg-muted/60 p-3 text-sm text-muted-foreground"><Settings2 className="size-4" /> Ansvarig: <span className="font-medium text-foreground">{settings.owner}</span></div></CardContent></Card></TabsContent>
          </Tabs>
        </main>
      </div>
    </div>
